...
prompt_dir = root_dir / "data" / "baseline_prompts"

# All prompt cases
generic_prompt = prompt_dir / "generic_prompts.json"
frog_prompt = prompt_dir / "frog_prompts.json"
generic_prompts_base = prompt_dir / "generic_prompts_base.json"
frog_prompts_base = prompt_dir / "frog_prompts_base.json"
...
MAX_NEW_TOKENS = 200
NUM_GENERATIONS = 40  # Number of generations per prompt for consistency analysis
...
def main(track_entropy: bool = True,
    ...
    if prompt_case:
        ...
        output_name = f"olmo7b_results_{prompt_case}.jsonl"
    else:
        selected_prompt_cases = PROMPT_CASES
        output_name = "olmo7b_results.jsonl"
    print(f"Selected prompt cases: {selected_prompt_cases}, saving retults to: {output_name}")
    with open(output_dir / output_name, "w", encoding="utf-8") as out_file:
        for model_key in MODELS.keys(): 
            ...
            for prompt_case in selected_prompt_cases:
                if prompt_case == "generic":
                    if model_type == 'base':
                        prompts_df = pd.read_json(generic_prompts_base, orient="records")
                    else:
                        prompts_df = pd.read_json(generic_prompt, orient="records")
                elif prompt_case == "frog":
                    if model_type == 'base':
                        prompts_df = pd.read_json(frog_prompts_base, orient="records")
                    else:
                        prompts_df = pd.read_json(frog_prompt, orient="records")
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_case",
                        ...
                        default=None,
                        help="Run only one prompt case (generic or frog).")
