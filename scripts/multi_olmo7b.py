...
def main(model_key: str,
    ...
    """Run analysis for one specific model at a time with optional entropy tracking."""

    # Select model for this parallel task
    model_name = MODELS[model_key]

    # Prepare output directory
    output_dir = data_dir / "olmo7b_results"
    output_dir.mkdir(exist_ok=True)    
    output_path = output_dir / f"olmo7b_{model_key}_results.jsonl"
    ...
    print(f"--- STARTING TASK: {model_key} ---")
    print(f"Loading from NVMe: {models_dir}")
    ...
    with open(output_path, "w", encoding="utf-8") as out_file:
        ...
        for prompt_data in prompts_to_process:
            ...
            print(f"\n[{model_key}] Processing profile_id: {profile_id} --------")
            
            for n in range(1, num_gens + 1):
                print(f"Generating {model_key} response {n}/{num_gens} ...")
...
if __name__ == "__main__":
    ...
    main(model_key=args.model_key, num_prompts=args.num_prompts)